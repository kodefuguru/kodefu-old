(function($) {
    ...
    $.extend($t, {
        datepicker: function(element, options) {
            ...
            if (this.onOpen)
                $element.bind('openDatePicker', this.onOpen);

            if (this.onClose)
                $element.bind('closeDatePicker', this.onClose);

            if (this.onChange)
                $element.bind('changeDatePicker', this.onChange);

            if (this.onLoad)
                $element.bind('loadDatePicker', this.onLoad).trigger('loadDatePicker')
    ...
    $.extend($t.datepicker.prototype, {
        $calendar: function() {

            if (!sharedCalendar) {

                sharedCalendar =
                    $($t.calendar.html(this.focusedDate, this.selectedDate, this.minDate, this.maxDate))
                        .hide()
            ...
            if (sharedCalendar.data('associatedDatePicker') != this.element) {
                ...
                $(sharedCalendar).css(elementPosition);
                sharedCalendar
                    .unbind('changeCalendar')
                    .bind('changeCalendar', $t.bind(this, this.onCalendarSelectionChange))
                    .unbind('navigate.Calendar')
                    .bind('navigate.Calendar', $t.bind(this, this.viewedMonthChanged))
            ...
            if (!sharedCalendar.is(':visible') && calendar.viewedMonth.value - this.focusedDate.value != 0) {
                ...
                if (this.selectedDate !== null)
                    calendar.value(this.selectedDate.value);
            }
        ...
        value: function(date) {
            if (date) {
                this.focusedDate = this.selectedDate = date.value ? date : new $t.datetime(date);
                $('.t-input', this.element).val($t.calendar.formatDate(this.selectedDate.toDate(), this.dateFormat));
        ...
        onCalendarSelectionChange: function(e) {
        ...
        checkSelectedDate: function(selectedDate, newlySelectedDate) {
            if (!selectedDate || (selectedDate.value > newlySelectedDate.value || newlySelectedDate.value > selectedDate.value)) {
                ...
                if ($t.trigger(this.element, 'changeDatePicker', {
        ...
        show: function() {
            $t.trigger(this.element, 'openDatePicker');
            this.showPopup();
        },

        hide: function() {
            $t.trigger(this.element, 'closeDatePicker');
            this.hidePopup();
        },
        ...
        keyDown: function(e) {
            ...
            if ($('.t-overlay', this.$calendar()).length > 0)
                return;
        ...
        parseDate: function(value) {
            var result = null;
            if (value != "") {
                result = this.parse(value, this.dateFormat);
                ...
                $('.t-input', this.element)
                    ...
                    .val(isNull ? value : $t.calendar.formatDate(result.toDate(), this.dateFormat));
            } else {
        ...
        parse: function(value, format, today) {
            ...
            if (dateCheck.test(value)) {
                date = this.parseMachineDate(value, format);
            } else {
                date = this.parseByToken(value, today);
            }
            return date;
        },

        parseMachineDate: function(value, format) {
            format = format || this.dateFormat; // required for unit tests
            var year = -1;
            var month = -1;
            var day = -1;
    ...
    $.fn.tDatePicker.defaults = {
        ...
        selectedDate: null,
        dateFormat: $t.cultureInfo.shortDate,
