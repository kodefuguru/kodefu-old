(function($) {
    ...
    $.extend($t, {
        panelbar: function(element, options) {
            ...
            if (this.onExpand)
                $element.bind('expandPanelBar', this.onExpand);

            if (this.onCollapse)
                $element.bind('collapsePanelBar', this.onCollapse);

            if (this.onSelect)
                $element.bind('selectPanelBar', this.onSelect);

            if (this.onError)
                $element.bind('errorPanelBar', this.onError);

            if (this.onLoad) {
                $element.bind('loadPanelBar', this.onLoad);
                $t.trigger($element, 'loadPanelBar');
            }

            var $content = $('li.t-state-active', this.element).find('> .t-content');
    ...
    $.extend($t.panelbar.prototype, {
        ...
        _click: function(e, element) {
            ...
            if ($t.trigger(this.element, 'selectPanelBar', { item: $item[0] })) {
                e.preventDefault();
            }
            ...
            if (contents.length != 0) {
                ...
                if (!visibility) {
                    $t.trigger(this.element, 'expandPanelBar', { item: $item[0] });
                } else {
                    $t.trigger(this.element, 'collapsePanelBar', { item: $item[0] });
                }
        ...
        _ajaxRequest: function($element, contentElement, isVisible) {
            ...
            $.ajax({
                ...
                error: $.proxy(function(xhr, status) {
                    if ($t.ajaxError(this.element, 'errorPanelBar', xhr, status))
                        return;
                }, this),
