(function($) {
    ...
    $t.grid = function(element, options) {
        ...
        if (this.onError)
            $(element).bind('error', this.onError);

        if (this.onDataBinding)
            $(element).bind('dataBinding', this.onDataBinding);

        if (this.onDataBound)
            $(element).bind('dataBound', this.onDataBound);

        if (this.onRowSelected)
            $(element).bind('rowSelected', this.onRowSelected);

        if (this.onRowDataBound)
            $(element).bind('rowDataBound', this.onRowDataBound);
        ...
        if (this.onLoad)
            $(this.element).bind('load', this.onLoad).trigger('load');
    }
    ...
    $t.grid.prototype = {
        ...
        bindData: function(data, html, groups) {
            ...
            for (var rowIndex = 0; rowIndex < dataLength; rowIndex++) {
                ...
                for (var i = 0, l = this.columns.length; i < l; i++) {
                    ...
                    if (column.commands) {
                        if ($.inArray('edit', column.commands) > -1)
                            html.cat('<a class="t-grid-action t-button t-state-default t-grid-edit" href="#">Edit</a>');
                        if ($.inArray('delete', column.commands) > -1)
                            html.cat('<a class="t-grid-action t-button t-state-default t-grid-delete" href="#">Delete</a>');
                    }
                }
            }
        },
        ...
        bindTo: function(data) {
            ...
            //innerHTML of table elements is readonly In IE. However jQuery.fn.html is two times slower than setting innerHTML in other browsers.
            if ($.browser.msie)
                this.$tbody.html(html.string());
            else
                this.$tbody[0].innerHTML = html.string();
