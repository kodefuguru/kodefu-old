(function($) {
    ...
    $.extend($t, {
        tabstrip: function(element, options) {
            ...
            // client events
            if (this.onSelect)
                $element.bind('selectTabStrip', this.onSelect);

            if (this.onError)
                $element.bind('errorTabStrip', this.onError);

            if (this.onLoad) {
                $element.bind('loadTabStrip', this.onLoad)
                $t.trigger($element, 'loadTabStrip');
            }

            var selectedItems = $('li.t-state-active', this.element);
            var content = this.getContentElement($('> .t-content', this.element), selectedItems.parent().children().index(selectedItems));
    ...
    $.extend($t.tabstrip.prototype, {
        ...
        _click: function(e, element) {
            ...
            if ($t.trigger(this.element, 'selectTabStrip', { item: $item[0], contentElement: content })) {
                e.preventDefault();
            }
        ...
        ajaxRequest: function($element, contentElement, complete) {
            ...
            $.ajax({
                ...
                error: $.proxy(function(xhr, status) {
                    if ($t.ajaxError(this.element, 'errorTabStrip', xhr, status))
                        return;
                }, this),
