(function($) {
    ...
    $.extend($t, {
        calendar: function(element, options) {
            ...
            if (this.onChange)
                $(this.element).bind('changeCalendar', this.onChange);

            /* header */
            $('.t-nav-next:not(.t-state-disabled)', element)
                .live('click', $t.bindToJavscriptAction(this, this.navigateToFuture));

            $('.t-nav-prev:not(.t-state-disabled)', element)
                .live('click', $t.bindToJavscriptAction(this, this.navigateToPast));

            $('.t-nav-fast:not(.t-state-disabled)', element)
                .live('click', $t.bindToJavscriptAction(this, this.navigateUp));
            ...
            $('td:not(.t-state-disabled):has(.t-link)', element)
                ...
                .live('click', $t.bind(this, this.navigateDown));

            if (this.onLoad)
                $(this.element).bind('loadCalendar', this.onLoad).trigger('loadCalendar')
        }
    });
    ...
    $.extend($t.calendar.prototype, {
        ...
        updateSelection: function() {
            ...
            var lastDayOfMonth = new $.telerik.datetime(firstDayOfMonth.value).date(32).date(0);

            if (!$t.calendar.isInRange(this.selectedDate, firstDayOfMonth, lastDayOfMonth))
            ...
            if (this.selectedDate !== null) {
                var me = this;
                $('.t-content td:not(.t-other-month)', this.element)
        ...
        value: function() {
            ...
            if (arguments.length == 1)
                this.selectedDate = arguments[0].value ? arguments[0] : new $t.datetime(arguments[0]);
        ...
        navigateVertically: function(viewIndex, viewedMonth, plunge, target) {
            ...
            var newView =
                $('<table class="t-content" cellspacing="0" summary=""></table>')
            ...
            if (plunge) {
                ...
                outerAnimationContainer =
                    $.telerik.fx._wrap(newView)
                        .css({
                            overflow: 'hidden',
                            position: 'relative'
                        });

                var coordinatesMod = $t.calendar.views[viewIndex].verticalDate(this.viewedMonth);
            ...
            $t.trigger(this.element, 'navigate.Calendar', {
                direction: plunge
            });
        },
        ...
        navigateDown: function(e, target, viewIndex) {
            ...
            if (currentViewIndex == 0) {
                ...
                if (!this.selectedDate || (this.selectedDate.value > newlySelectedDate.value || newlySelectedDate.value > this.selectedDate.value)) {
                    if ($t.trigger(this.element, 'changeCalendar', {
    ...
    $.extend($t.calendar, {
        views: [{
            ...
            body: function(viewedMonth, minDate, maxDate, selectedDate, urlFormat, dates) {
                ...
                for (var weekRow = 0; weekRow < 6; weekRow++) {
                    ...
                    for (var day = 0; day < 7; day++) {
                        ...
                        html.cat('<td').cat(cellClass != '' ? ' class="' + cellClass + '"' : '').cat('>');
            ...
            firstLastDay: function(date, isFirstDay, calendar) {
                return isFirstDay ? $t.datetime.firstDayOfMonth(date) :
                                    new $t.datetime(date.year(), date.month() + 1, 0);
            },
            navCheck: function(date1, date2, isBigger) {
                return isBigger ?
                           (new $t.datetime(date2.year(), date2.month() + 1, date2.date()).value - date1.value) <= 0
            ...
            {   /*Year*/
                ...
                firstLastDay: function(date, isFirstDay) {
                    return isFirstDay ? new $t.datetime(date.year(), 0, 1) :
                                        new $t.datetime(date.year(), 11, 1);
                },
                navCheck: function(date1, date2, isBigger) {
            ...
            {   /*Decade*/
                ...
                body: function(viewedMonth, minDate, maxDate) {
                    return $t.calendar.metaView(false, viewedMonth, function() {
                        ...
                        var currentYear = viewedMonth.year() - viewedMonth.year() % 10 - 1;
                ...
                firstLastDay: function(date, isFirstDay) {
                    return isFirstDay ? new $t.datetime(date.year() - (date.year() % 10), 0, 1) :
                                        new $t.datetime(date.year() - (date.year() % 10) + 9, 0, 1);
                },
                navCheck: function(date1, date2, isBigger) {
        ...
        html: function(viewedMonth, selectedDate, minDate, maxDate, urlFormat, dates) {
            ...
            var html = new $t.stringBuilder();
            html.cat('<div class="t-widget t-calendar">')
                ...
                .cat('</div>')
                .cat('<table summary="" class="t-content" cellspacing="0">')
        ...
        metaView: function(isYearView, viewedMonth, getCollection) {
            ...
            for (var i = 0, len = collection.length; i < len; i++) {
                ...
                html.cat('<td')
                if ((i == 0 || i == len - 1) && isYearView === false) html.cat(' class="t-other-month"')
                html.cat('>')
        ...
        findTarget: function(focusedDate, viewedIndex, calendar, isFuture) {
            var findTarget = function(collection, searchedText) {
                for (var i = 0, j = collection.length; i < j; i++)
                    if ($(collection[i].children[0]).text().indexOf(searchedText) != -1)
                    return collection[i];
            }
            ...
            var $target;
            if (viewedIndex == 0) {
                var collection = cells.filter(':not(.t-other-month)');
                $target = $(findTarget(collection, focusedDate.date()));
